Shader "CustomPP/Dither" {
    ...
    SubShader {
        ...
        CGINCLUDE
            ...
            Texture2D _MainTex;
            SamplerState point_clamp_sampler = sampler_state { Filter = Point; AddressU = Clamp; AddressV = Clamp; };
        ...
        Pass {
            ...
            fixed4 fp(v2f i) : SV_Target {
                ...
                // Calculate color quantization factor
                float quantization = _ColorCount - 1.0f;
            
                // Apply posterization effect
                output.rgb = round(output.rgb * quantization) / quantization;
                //output = floor((_ColorCount - 1.0f) * output + 0.5) / (_ColorCount - 1.0f);
            
                // Prevent colors from going full black
                output.rgb = max(output.rgb, 0.0015f);
                //output.rgb = min(output.rgb, 3.f);
                output.rgb = float4(1.0f, 0.0f, 0.0f, 1.0f);
                return output;
            }
            ENDCG
        }
